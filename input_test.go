...
func TestDeletePrevWord(t *testing.T) {
	tests := []struct {
		...
		{
			name:         "delete with multiple spaces",
			text:         "hello   world",
			pos:          13,
			expectedText: "hello   ",
			expectedPos:  8,
		...
		{
			name:         "delete in middle of word",
			text:         "hello world",
			pos:          9,
			expectedText: "hello ld",
			expectedPos:  6,
		},
		{
		...
		{
			name:         "delete three words",
			text:         "error not found",
			pos:          15,
			expectedText: "error not ",
			expectedPos:  10,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			resultText, resultPos := deletePrevWord(tt.text, tt.pos)
			if resultText != tt.expectedText || resultPos != tt.expectedPos {
				t.Errorf("deletePrevWord(%q, %d) = (%q, %d), want (%q, %d)",
					tt.text, tt.pos, resultText, resultPos, tt.expectedText, tt.expectedPos)
			}
		})
	}
}
